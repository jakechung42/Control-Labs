...
s = tf('s');
Kp = 7;
ol_tf = 1.437e6/(s^3+484.5*s^2+38025*s+2.53e5)
...
raw_step = csvread(path, 2, 0);
time_step_scope_15 = raw_step(:,1);
V_in_step_scope_15 = raw_step(:,2);
V_out_step_scope_15 = raw_step(:,3);
...
t_step_mod_15 = linspace(min(time_step_scope_15), max(time_step_scope_15), length(time_step_scope_15));
...
[step_model_15, t_step_model_15] = lsim(model, V_in_step_scope_15, t_step_mod_15);
...
ylabel('Voltage (V)')
xlabel('Time (s)')
title('Comparing theoretical step response of Kp = 1 model with Kp = 7 response data')

% Kp 15
figure
plot(t_step_model_15, step_model_15)
hold on
plot(time_step_scope_15, V_out_step_scope_15, '-')
grid on
ylabel('Voltage (V)')
xlabel('Time (s)')
title('Comparing theoretical step response of Kp = 1 model with Kp = 15 response data')