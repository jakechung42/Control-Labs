...
volatile float Control_km1, Error_km1, Control_km2, Error_km2;
...
void setup()
{
  ...
  samplingFreq = 1000; //define sampling frequency in Hz
...
void ControlLoop()
{
  ...
  Sensor_Input = adc->adc0->analogRead(A1);          				  // Read Analog value channel A1 (AD) - Sensor Input (0-3.3 volts) after the B2SE circuit
	...
	Control = 1.429*Error - 2.824*Error_km1 + 1.395*Error_km2 + 1.956*Control_km1 - 0.956*Control_km2;   // Control (units are voltage  +- 10 volts)  This equation impliments the control equation

  //update variables
  Control_km1 = Control;
  Control_km2 = Control_km1;
  Error_km1 = Error;
  Error_km2 = Error_km1;
