...
void setup()
{
  ...
  Kp          = 0.547;   	// Proportional control constant - must be calculated from your controller design and set properly here
  Ki          = 2.1935;		// Integral control constant - must be calculated from your controller design and set properly here
  Ts          = .000042;		// Sampling time (seconds) - must be measured and set properly here
}
...
void loop()
{
    ...
    Sensor_Input = adc->adc0->analogRead(A1);          				  // Read Analog value channel A1 (AD) - Sensor Input (0-3.3 volts) after the B2SE circuit
	  ...
	  Control = (-Kp + 1./2.*Ki*Ts)*Error_m1 + (Kp + 1./2.*Ki*Ts)*Error + Control_m1;	// Control (units are voltage  +- 10 volts)  This equation impliments the control equation (PI Control)
	...
	// Note that only Error_m1 and Control_m1 are needed for the control equation above but a more complicated control difference equation may require more previous values
